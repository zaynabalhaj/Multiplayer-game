...
import time
pygame.init()
screen=pygame.display.set_mode((789,567))
...
text_surface = test_font.render("Type Runner",False,"Grey")
...
Server_Port = 8000
Server_IP = '127.0.0.1' # We Should Put the IP Address of the Server Computer Here
happened = False
while True:
    ...
    if happened and tk==1:
        ...
        if (message[1]=="0"):  # welcome message
            ...
            pygame.display.update()
        if (message[1]=="1"):#game prompt
            server_command=proj_font.render(message[0],False,"Grey")
            screen.blit(ICB_surface,(0,0))    
            screen.blit(server_command,(50,117))
            pygame.display.update()
            tk=0
        if (message[1]=="2"):#disqualified
            server_command=proj_font.render(message[0],False,"Grey")
            tk=1
            screen.blit(ICB_surface,(0,0))    
            screen.blit(server_command,(20,117))
            pygame.display.update()
        if (message[1]=="3"):#round leaderboard
            server_command=proj_font.render(message[0],False,"Grey")
            tk=1
            screen.blit(ICB_surface,(0,0))    
            screen.blit(server_command,(10,117))
            pygame.display.update()
        if (message[1]=="4"):#final leaderboard
            client.close()
            while True:
                for event in pygame.event.get():
                    if event.type==pygame.QUIT:
